...
internal class Program
{
    ...
    private static ulong PartTwo(string[] almanac)
    {
        ulong lowestNum = 9999999999;

        var maps = new Dictionary<int, List<(ulong dest, ulong src, ulong count)>>();
        var seeds = Regex.Matches(almanac[0], @"\d+").Select(m => ulong.Parse(m.Value)).ToList();
        var seedRanges = new List<(ulong start, ulong count)>();
        ...
        {
            foreach (var seed in Range(seedRange.start, seedRange.count))
            {
                var currMap = 1;
                var currKey = seed;
                while (currMap < maps.Count + 1)
    ...
    private static void Main(string[] args)
    {
        ...
        Console.WriteLine($"Part Two: {PartTwo(puzzleInput)}"); // Answer: 382895070
    }
}
